package collaboRhythm.hiviva.view.components
{
	...
	public class TolerabilityChartReport extends FeathersControl
	{
			...
			for (var dayCount:int = 0; dayCount < this._dayRange; dayCount++)
			{
				...
				if (dayTime >= earliestSchedule && dayTime < latestSchedule)
				{
					...
					if (columnData != null)
					{
						...
						{
							// which medicine has the highest tolerability
							if (tolerability < Number(columnData[i].data.Tolerability))
							{
								tolerability = Number(columnData[i].data.Tolerability);
							}
						}
						if (tolerability < this._lowestValue) this._lowestValue = tolerability;
					}
					else
					{
